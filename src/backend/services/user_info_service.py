...
import discord
from src.backend.db.db_reader_writer import DatabaseReader, DatabaseWriter, Database
...
def create_user_embed_field(
    user_info: Dict[str, Any], title: str = "User Information"
) -> Dict[str, Any]:
    """
    ...
    return {"name": title, "value": user_text, "inline": False}
...
class UserInfoService:
    ...
    def ensure_player_exists(
        ...
        if player is None:
            ...
            self.create_player(
                discord_uid=discord_uid, discord_username=discord_username
            )
            player = self.get_player(discord_uid)
        else:
    ...
    def update_player(
        ...
        if success and log_changes and old_player:
            ...
            if (
                ...
                and old_player.get("discord_username") != discord_username
            ):
            ...
            if alt_player_name_1 is not None:
                ...
                normalized_new = (
                    alt_player_name_1.strip() if alt_player_name_1 else None
                )
            ...
            if alt_player_name_2 is not None:
                # Normalize: empty string becomes None for comparison
                normalized_new = (
                    alt_player_name_2.strip() if alt_player_name_2 else None
                )
    ...
    def update_country(self, discord_uid: int, country: str) -> bool:
        ...
        if success and player:
            ...
            player_name = (
                player.get("player_name") or player.get("discord_username") or "Unknown"
            )
            self.writer.log_player_action(
                discord_uid=discord_uid,
                player_name=player_name,
    ...
    def submit_activation_code(
        ...
        if player:
            ...
            success = self.writer.update_player_activation_code(
                discord_uid, activation_code
            )
            if success:
                # Use player_name if available, otherwise use discord_username, fallback to "Unknown"
                player_name = (
            ...
            player_id = self.create_player(
                discord_uid=discord_uid, activation_code=activation_code
            )
            if player_id:
                return {
                    "status": "ok",
                    "message": "Player created with activation code",
                }
            else:
