...
class MatchCompletionService:
    ...
    def start_monitoring_match(
        self, match_id: int, on_complete_callback: Optional[Callable] = None
    ):
        """
    ...
    async def wait_for_match_completion(
        self, match_id: int, timeout: int = 30
    ) -> Optional[dict]:
        """
        ...
        try:
            ...
            try:
                ...
                print(
                    f"⏰ WAIT: Match {match_id} completion timed out after {timeout} seconds"
                )
                return None
    ...
    async def check_match_completion(self, match_id: int):
        ...
        async with lock:
            try:
                ...
                print(
                    f"🔍 CHECK: Match {match_id} reports: p1={p1_report}, p2={p2_report}, result={match_result}"
                )

                # Abort if any report is -3 (initiator) or both are -1/-3 with match_result=-1
                if (p1_report == -3 or p2_report == -3) or (
                    match_result == -1
                    and (p1_report in (-3, -1) and p2_report in (-3, -1))
                ):
                ...
                if p1_report is not None and p2_report is not None:
                    ...
                    print(
                        f"📝 CHECK: Match {match_id} still waiting for reports: p1={p1_report}, p2={p2_report}"
                    )
            ...
            while match_id in self.monitored_matches:
                ...
                async with lock:
                    ...
                    if p1_report is not None and p2_report is not None:
                        ...
                        if match_data.get("player_1_report") == match_data.get(
                            "player_2_report"
                        ):
                            await self._handle_match_completion(match_id, match_data)
                        else:
    ...
    async def _handle_match_completion(self, match_id: int, match_data: dict):
        ...
        try:
            ...
            if not final_match_data:
                print(
                    f"❌ Could not get final results for match {match_id} during handling."
                )
                return
    ...
    async def _handle_match_abort(self, match_id: int, match_data: dict):
        ...
        try:
            ...
            print(
                f"  -> Notifying {len(callbacks)} callbacks for match {match_id} abort."
            )
            for callback in callbacks:
                try:
                    await callback(
                        status="abort",
                        data={"match_id": match_id, "match_data": final_match_data},
                    )
                except Exception as e:
                    print(
                        f"❌ Error executing abort callback for match {match_id}: {e}"
                    )
    ...
    async def _handle_match_conflict(self, match_id: int):
        ...
        try:
            ...
            # Clean up monitoring for this match
            print(
                f"🧹 CLEANUP: Unregistering and stopping monitoring for conflict match {match_id}"
            )
    ...
    async def _notify_players_match_complete(self, match_id: int, final_results: dict):
        ...
        callbacks = self.notification_callbacks.pop(match_id, [])
        print(
            f"  -> Notifying {len(callbacks)} callbacks for match {match_id} completion."
        )
        for callback in callbacks:
            try:
                ...
                print(
                    f"❌ Error executing completion callback for match {match_id}: {e}"
                )
    ...
    async def _notify_players_of_conflict(self, match_id: int):
        ...
        callbacks = self.notification_callbacks.pop(match_id, [])
        print(
            f"  -> Notifying {len(callbacks)} callbacks for match {match_id} conflict."
        )
        for callback in callbacks:
            try:
    ...
    async def _get_match_final_results(self, match_id: int) -> Optional[dict]:
        ...
        try:
            ...
            p1_info = self.db_reader.get_player_by_discord_uid(
                match_data["player_1_discord_uid"]
            )
            p2_info = self.db_reader.get_player_by_discord_uid(
                match_data["player_2_discord_uid"]
            )

            p1_name = p1_info.get(
                "player_name", str(match_data["player_1_discord_uid"])
            )
            p2_name = p2_info.get(
                "player_name", str(match_data["player_2_discord_uid"])
            )
            ...
            result_text = result_text_map.get(
                match_data["match_result"], "Undetermined"
            )
