...
class StorageService:
    ...
    def upload_replay(
        self, match_id: int, player_discord_uid: int, file_data: bytes, filename: str
    ) -> Optional[str]:
        """
        ...
        if len(file_data) > max_size:
            print(
                f"[Storage] ERROR: File too large: {len(file_data)} bytes (max {max_size})"
            )
            return None
    ...
    def download_replay(
        self, match_id: int, player_discord_uid: int
    ) -> Optional[bytes]:
        """
    ...
    def get_replay_url(self, match_id: int, player_discord_uid: int) -> str:
        """
    ...
    def delete_replay(self, match_id: int, player_discord_uid: int) -> bool:
        """
