...
class StaticDataCache:
    ...
    def _load_from_json(self) -> None:
        ...
        with open(os.path.join(base_path, 'countries.json'), 'r', encoding='utf-8') as f:
...
class PlayerRecordCache:
    ...
    def get(self, discord_uid: int) -> Optional[Dict]:
        """
        Get a player record from cache if available and not expired.
        
        Args:
            discord_uid: Discord user ID
            
        Returns:
            Player record dictionary or None if not in cache/expired
        """
        import time
        
        if discord_uid not in self._cache:
            self._misses += 1
            return None
        
        # Check if expired
        if time.time() - self._timestamps[discord_uid] > self._ttl:
            # Expired, remove from cache
            del self._cache[discord_uid]
            del self._timestamps[discord_uid]
            self._misses += 1
            return None
        
        self._hits += 1
        return self._cache[discord_uid].copy()
    
    def set(self, discord_uid: int, player_record: Dict) -> None:
        """
        Store a player record in cache.
        
        Args:
            discord_uid: Discord user ID
            player_record: Player record dictionary
        """
        import time
        
        self._cache[discord_uid] = player_record.copy()
        self._timestamps[discord_uid] = time.time()
    
    def invalidate(self, discord_uid: int) -> None:
        """
        Remove a player record from cache.
        
        Args:
            discord_uid: Discord user ID to invalidate
        """
        self._cache.pop(discord_uid, None)
        self._timestamps.pop(discord_uid, None)
...
# Global singleton instance
player_cache = PlayerRecordCache()
