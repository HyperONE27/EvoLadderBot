...
class Player:
    def __init__(
        ...
        bw_mmr: Optional[int] = None,
        sc2_mmr: Optional[int] = None,
    ):
        ...
        self.has_bw_race = any(
            race.startswith("bw_") for race in preferences.selected_races
        )
        self.has_sc2_race = any(
            race.startswith("sc2_") for race in preferences.selected_races
        )

        # Get the specific race codes
        self.bw_race = next(
            (race for race in preferences.selected_races if race.startswith("bw_")),
            None,
        )
        self.sc2_race = next(
            (race for race in preferences.selected_races if race.startswith("sc2_")),
            None,
        )
...
class Matchmaker:
    ...
    async def add_player(self, player: Player) -> None:
        ...
        for race in player.preferences.selected_races:
            ...
            if mmr_data:
                ...
                self.db_writer.create_or_update_mmr_1v1(
                    player.discord_user_id, player.user_id, race, default_mmr
                )
                if race.startswith("bw_"):
        ...
        async with self.lock:
            print(
                f"👤 {player.user_id} (Discord ID: {player.discord_user_id}) joined the queue"
            )
    ...
    async def remove_player(self, discord_user_id: int) -> None:
        ...
        async with self.lock:
            ...
            self.players = [
                p for p in self.players if p.discord_user_id != discord_user_id
            ]
    ...
    def set_match_callback(
        self, callback: Callable[[MatchResult, Callable[[Callable], None]], None]
    ) -> None:
    ...
    def max_diff(self, wait_cycles: int) -> int:
        ...
        if effective_pop == 0:
            ...
            if pressure_ratio >= self.HIGH_PRESSURE_THRESHOLD:  # High pressure
                ...
                pressure_ratio >= self.MODERATE_PRESSURE_THRESHOLD
            ):  # Moderate pressure
    ...
    def equalize_lists(
        self, list_x: List[Player], list_y: List[Player], list_z: List[Player]
    ) -> Tuple[List[Player], List[Player], List[Player]]:
        """
    ...
    def find_matches(
        self, lead_side: List[Player], follow_side: List[Player], is_bw_match: bool
        ...
        lead_mean = sum(p.get_effective_mmr(is_bw_match) or 0 for p in lead_side) / len(
            lead_side
        )
    ...
    async def attempt_match(self):
        ...
        original_bw_only, original_sc2_only, original_both_races = (
            self.categorize_players(current_players)
        )

        print(
            f"   📊 Queue composition: BW-only={len(original_bw_only)}, SC2-only={len(original_sc2_only)}, Both={len(original_both_races)}"
        )
        ...
        bw_list, sc2_list, remaining_z = self.equalize_lists(
            bw_list, sc2_list, both_races
        )

        print(
            f"   📊 After equalization: BW={len(bw_list)}, SC2={len(sc2_list)}, Remaining Z={len(remaining_z)}"
        )
        ...
        if len(bw_list) > 0 and len(sc2_list) > 0:
            ...
            print(
                f"   ✅ Found {len(bw_matches)} BW vs SC2 matches (lead: {len(lead_side)}, follow: {len(follow_side)})"
            )
        ...
        for p1, p2 in matches:
            ...
            from src.backend.services.match_completion_service import (
            ...
            if self.match_callback:
                ...
                self.match_callback(
                    ...
                    register_completion_callback=lambda callback: match_completion_service.start_monitoring_match(
                        match_id, on_complete_callback=callback
                    ),
                )
            else:
        ...
        await self._update_queue_after_matching(
            matched_players, original_bw_only, original_sc2_only, original_both_races
        )
    ...
    def _prune_recent_activity(self):
        ...
        stale_players = [
            uid
            for uid, timestamp in self.recent_activity.items()
            if now - timestamp > self.ACTIVITY_WINDOW_SECONDS
        ]
        for uid in stale_players:
            del self.recent_activity[uid]

    async def _update_queue_after_matching(
        self,
        matched_players: set,
        original_bw_only: List[Player],
        original_sc2_only: List[Player],
        original_both_races: List[Player],
    ):
        """
        ...
        remaining_bw = [
            p for p in original_bw_only if p.discord_user_id not in matched_players
        ]
        remaining_sc2 = [
            p for p in original_sc2_only if p.discord_user_id not in matched_players
        ]
        remaining_both = [
            p for p in original_both_races if p.discord_user_id not in matched_players
        ]

        print(
            f"   📊 Remaining players: BW-only={len(remaining_bw)}, SC2-only={len(remaining_sc2)}, Both={len(remaining_both)}"
        )
    ...
    async def run(self):
        ...
        while self.running:
            ...
            if len(self.players) > 0:
                print(
                    f"⏰ Checking for matches with {len(self.players)} players in queue..."
                )
    ...
    def record_match_result(
        self, match_id: int, player_discord_uid: int, report_value: int
    ) -> bool:
        """
        Record a player's report for a match. This is the single entry point for any report.
        """
        success = self.db_writer.update_player_report_1v1(
            match_id, player_discord_uid, report_value
        )
        if not success:
        ...
        print(
            f"🚀 Triggering immediate completion check for match {match_id} after a report."
        )
    ...
    def abort_match(self, match_id: int, player_discord_uid: int) -> bool:
        ...
        if was_aborted:
            ...
            print(
                f"🚀 Triggering immediate completion check for match {match_id} after abort."
            )
            asyncio.create_task(
    ...
    async def _calculate_and_write_mmr(self, match_id: int, match_data: dict) -> bool:
        ...
        if p1_report is None or p2_report is None:
            print(
                f"📝 Player report recorded for match {match_id}, waiting for other player"
            )
            return True
        ...
        if match_result == -2:
            print(
                f"⚠️ Conflicting reports for match {match_id}, manual resolution required"
            )
            return True
        ...
        if p1_race and p2_race:
            ...
            if p1_mmr_data and p2_mmr_data:
                ...
                mmr_outcome = mmr_service.calculate_new_mmr(
                    p1_current_mmr, p2_current_mmr, result
                )
                ...
                print(
                    f"   Player 1 ({player1_uid}): {p1_current_mmr} → {mmr_outcome.player_one_mmr} ({p1_race})"
                )
                print(
                    f"   Player 2 ({player2_uid}): {p2_current_mmr} → {mmr_outcome.player_two_mmr} ({p2_race})"
                )
