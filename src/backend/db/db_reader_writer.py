...
class Database:
    """
    Handles database connections and adapter instantiation.
    """
    ...
    def __init__(self, pool: Optional[ConnectionPool] = None):
        ...
        Args:
            pool: An optional ConnectionPool for PostgreSQL.
        """
        self.db_type = DATABASE_TYPE
        self.adapter = get_adapter(self.db_type, pool=pool)

        print(f"[Database] Initialized with adapter: {self.db_type}")
        if self.db_type == "postgresql":
            print(f"[Database] Connection: {self.adapter.get_connection_string()}")
...
class DatabaseReader:
    ...
    def get_player_by_activation_code(
    ...
    def get_player_action_logs(
        self, discord_uid: Optional[int] = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
    ...
    def get_player_mmr_1v1(
        self, discord_uid: int, race: Optional[str] = None
    ) -> Optional[Dict[str, Any]]:
    ...
    def count_leaderboard_1v1(
        self, race: Optional[str] = None, country: Optional[str] = None
    ) -> int:
    ...
    def get_player_matches_1v1(
        self, discord_uid: int, limit: int = 50
    ) -> List[Dict[str, Any]]:
    ...
    def get_match_1v1(self, match_id: int) -> Optional[Dict[str, Any]]:
        ...
        results = self.adapter.execute_query(
            "SELECT * FROM matches_1v1 WHERE id = :match_id", {"match_id": match_id}
        )
        return results[0] if results else None
...
class DatabaseWriter:
    ...
    def update_player(
        ...
        if alt_player_name_1 is not None:
            ...
            params["alt_player_name_1"] = (
                alt_player_name_1 if alt_player_name_1.strip() else None
            )

        if alt_player_name_2 is not None:
            updates.append("alt_player_name_2 = :alt_player_name_2")
            # Convert empty string to None for proper NULL storage
            params["alt_player_name_2"] = (
                alt_player_name_2 if alt_player_name_2.strip() else None
            )
        ...
        if accepted_tos is not None:
            ...
            if accepted_tos:
                updates.append(
                    "accepted_tos_date = COALESCE(accepted_tos_date, :accepted_tos_date)"
                )
        ...
        if completed_setup is not None:
            ...
            if completed_setup:
                updates.append(
                    "completed_setup_date = COALESCE(completed_setup_date, :completed_setup_date)"
                )
        ...
        query = (
            f"UPDATE players SET {', '.join(updates)} WHERE discord_uid = :discord_uid"
        )
    ...
    def update_player_activation_code(
        self, discord_uid: int, activation_code: str
    ) -> bool:
    ...
    def update_player_remaining_aborts(
        self, discord_uid: int, remaining_aborts: int
    ) -> bool:
    ...
    def log_player_action(
        ...
        self.adapter.execute_write(
            ...
            """,
            {"updated_at": get_timestamp(), "discord_uid": discord_uid},
        )
    ...
    def update_player_report_1v1(
        ...
        if reports_results:
            ...
            if (
                reports["player_1_report"] is not None
                and reports["player_2_report"] is not None
            ):
    ...
    def update_match_replay_1v1(
        self, match_id: int, player_discord_uid: int, replay_path: str, replay_time: str
    ) -> bool:
        """
    ...
    def update_match_mmr_change(self, match_id: int, mmr_change: int) -> bool:
        """
    ...
    def abort_match_1v1(
        self, match_id: int, player_discord_uid: int, abort_timer_seconds: int
    ) -> bool:
        """
        ...
        try:
            ...
            rowcount = self.adapter.execute_write(
                update_query,
                {"match_id": match_id, "abort_timer_seconds": abort_timer_seconds},
            )
    ...
    def insert_command_call(
        self, discord_uid: int, player_name: str, command: str
    ) -> bool:
        """
