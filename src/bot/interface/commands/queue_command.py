...
from src.bot.interface.components.error_embed import (
    ErrorEmbedException,
    create_error_view_from_exception,
)
...
# This is a simple way to prevent users from interacting with old views
# NOTE: This will reset on bot restart. A more persistent solution
# would involve storing this state in the database if needed.
ACTIVE_MATCH_VIEWS = set()
...
class MatchFoundViewManager:
    ...
    async def register(
        self, match_id: int, channel_id: int, view: "MatchFoundView"
    ) -> None:
        async with self._lock:
            ...
            # channel_to_match_view_map[channel_id] = view # This line is removed as per new_code
    ...
    async def unregister(self, match_id: int, channel_id: int) -> None:
        async with self._lock:
            ...
            # channel_to_match_view_map.pop(channel_id, None) # This line is removed as per new_code

    async def get_views_by_match_id(
        self, match_id: int
    ) -> list[tuple[int, "MatchFoundView"]]:
        async with self._lock:
...
# channel_to_match_view_map: Dict[int, "MatchFoundView"] = {} # This line is removed as per new_code
...
def register_queue_command(tree: app_commands.CommandTree):
    ...
    async def queue(interaction: discord.Interaction):
        ...
        try:
            player_record = command_guard_service.require_player_exists(
                interaction.user.id, interaction.user.name
            )
            command_guard_service.require_queue_access(player_record)
        ...
        matchmaker = interaction.client.app_context.matchmaking_service
        view = QueueView(
            races_service=interaction.client.app_context.races_service,
            maps_service=interaction.client.app_context.maps_service,
            user_info_service=interaction.client.app_context.user_info_service,
            matchmaking_service=matchmaker,
            player_record=player_record,
        )
        await view.load_preferences()
        await send_ephemeral_response(
            interaction, embed=create_queue_embed(), view=view
        )
...
async def queue_command(interaction: discord.Interaction):
    ...
    is_in_match_view = any(
        v.match_result.player_1_discord_id == interaction.user.id
        or v.match_result.player_2_discord_id == interaction.user.id
        for v in channel_to_match_view_map.values()
    )

    # Prevent multiple queue attempts or queuing while a match is active
    if (
        await queue_searching_view_manager.has_view(interaction.user.id)
        or is_in_match_view
    ):
        ...
        await send_ephemeral_response(
            interaction, embed=error_view.embed, view=error_view
        )
        return
...
class JoinQueueButton(discord.ui.Button):
    ...
    def __init__(self):
        super().__init__(
            label="Join Queue", emoji="🚀", style=discord.ButtonStyle.secondary, row=0
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Prevent multiple queue attempts or queuing while a match is active
        if (
            await queue_searching_view_manager.has_view(user_id)
            or interaction.user.id in match_results
        ):
            error = ErrorEmbedException(
                title="Queueing Not Allowed",
        ...
        await interaction.response.edit_message(
            embed=searching_view.build_searching_embed(), view=searching_view
        )
...
class CancelQueueSetupButton(discord.ui.Button):
    ...
    def __init__(self):
        super().__init__(
            label="Cancel", emoji="✖️", style=discord.ButtonStyle.danger, row=0
        )
...
class QueueView(discord.ui.View):
    ...
    def __init__(self, discord_user_id: int, default_races=None, default_maps=None):
        ...
        self.selected_bw_race = next(
            (race for race in default_races if race.startswith("bw_")), None
        )
        self.selected_sc2_race = next(
            (race for race in default_races if race.startswith("sc2_")), None
        )
    ...
    @classmethod
    async def create(
        cls, discord_user_id: int, default_races=None, default_maps=None
    ) -> "QueueView":
        view = cls(
            discord_user_id, default_races=default_races, default_maps=default_maps
        )
    ...
    async def persist_preferences(self) -> None:
        ...
        def _write_preferences() -> None:
            try:
                ...
                logger.error(
                    "Failed to update 1v1 preferences for user %s: %s",
    ...
    def get_embed(self):
        ...
        if selected_codes:
            ...
            race_list = "\n".join(details)
            embed.add_field(name="Selected Races", value=race_list, inline=False)
        else:
            embed.add_field(name="Selected Races", value="None selected", inline=False)
        ...
        if self.vetoed_maps:
            ...
            sorted_maps = [
                map_name for map_name in map_order if map_name in self.vetoed_maps
            ]
            # Add Discord number emotes
            number_emotes = [":one:", ":two:", ":three:", ":four:"]
            map_list = "\n".join(
                [
                    f"{number_emotes[i]} {map_name}"
                    for i, map_name in enumerate(sorted_maps)
                ]
            )
            embed.add_field(
                name=f"Vetoed Maps ({veto_count}/4)", value=map_list, inline=False
            )
        else:
            embed.add_field(name="Vetoed Maps (0/4)", value="No vetoes", inline=False)
...
class QueueSearchingView(discord.ui.View):
    ...
    def __init__(self, original_view, selected_races, vetoed_maps, player):
        ...
        self.selected_bw_race = next(
            (code for code in selected_races if code.startswith("bw_")), None
        )
        self.selected_sc2_race = next(
            (code for code in selected_races if code.startswith("sc2_")), None
        )
    ...
    async def periodic_status_update(self):
        while self.is_active:
            if not await queue_searching_view_manager.has_view(
                self.player.discord_user_id
            ):
                break
            ...
            async with self.status_lock:
                ...
                try:
                    await self.last_interaction.edit_original_response(
                        embed=self.build_searching_embed(), view=self
                    )
                except Exception:
                    pass
    ...
    def build_searching_embed(self) -> discord.Embed:
        ...
        next_wave_epoch = int(
            time.time()
            - (time.time() % matchmaker.MATCH_INTERVAL_SECONDS)
            + matchmaker.MATCH_INTERVAL_SECONDS
        )
        embed = discord.Embed(
            title="🔍 Searching...",
            description=(
    ...
    async def periodic_match_check(self):
        ...
        while self.is_active:
            ...
            if self.player.discord_user_id in match_results:
                ...
                is_player1 = (
                    match_result.player_1_discord_id == self.player.discord_user_id
                )
                ...
                await queue_searching_view_manager.unregister(
                    self.player.discord_user_id, view=self
                )
                break
    ...
    async def on_timeout(self):
        ...
        if await matchmaker.is_player_in_queue(self.player.discord_user_id):
            ...
            print(
                f"🚪 Player {self.player.user_id} timed out and was removed from queue."
            )
...
class CancelQueueButton(discord.ui.Button):
    ...
    def __init__(self, original_view, player):
        super().__init__(
            label="Cancel Queue", emoji="✖️", style=discord.ButtonStyle.danger, row=0
        )
        self.original_view = original_view
        self.player = player
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if existing_match:
            ...
            is_player1 = (
                existing_match.player_1_discord_id == self.player.discord_user_id
            )
            ...
            await queue_searching_view_manager.unregister(
                self.player.discord_user_id, view=parent_view
            )
        ...
        await interaction.response.edit_message(
            embed=self.original_view.get_embed(), view=self.original_view
        )
...
async def wait_for_match_completion(match_id: int, timeout: int = 30) -> Optional[dict]:
    ...
    try:
        ...
        final_results = await match_completion_service.wait_for_match_completion(
            match_id, timeout
        )
...
def handle_match_result(
    match_result: MatchResult, register_completion_callback: Callable[[Callable], None]
):
    """Handle when a match is found"""
    print(
        f"🎉 Match #{match_result.match_id}: {match_result.player_1_user_id} vs {match_result.player_2_user_id} | {match_result.map_choice} @ {match_result.server_choice}"
    )
...
class MatchFoundView(discord.ui.View):
    ...
    def __init__(self, match_result: MatchResult, is_player1: bool):
        super().__init__(
            timeout=None
        )  # No timeout - let match completion service handle cleanup
        ...
        if hasattr(match_result, "register_completion_callback"):
            match_result.register_completion_callback(
                self.handle_completion_notification
            )
        ...
        self.abort_deadline = get_current_unix_timestamp() + ABORT_TIMER_SECONDS
    ...
    async def disable_abort_after_delay(self):
        ...
        await asyncio.sleep(ABORT_TIMER_SECONDS)

        # Double-check if the button should still be active
        if self.abort_button.disabled or self.match_result.match_result in [
            "aborted",
            "conflict",
            "player1_win",
            "player2_win",
        ...
        async with self.edit_lock:
            if self.last_interaction:
                try:
                    ...
                    embed.add_field(
                        name="⚠️ Abort Window Closed",
                        value="The time to abort this match has expired.",
                        inline=False,
                    )
                    await self.last_interaction.edit_original_response(
        ...
        if self.match_result.replay_uploaded == "Yes":
            ...
            if self.selected_result:
                ...
                self.confirm_select.options = [
                    discord.SelectOption(
                        label=f"Confirm: {result_label}", value="confirm"
                    )
                ]
            ...
            self.result_select.placeholder = (
                "Upload replay file to enable result reporting"
            )
            self.confirm_select.disabled = True
    ...
    def get_embed(self) -> discord.Embed:
        ...
        p1_display_name = (
            p1_name if p1_name else str(self.match_result.player_1_discord_id)
        )
        ...
        p2_display_name = (
            p2_name if p2_name else str(self.match_result.player_2_discord_id)
        )
        ...
        server_display = regions_service.format_server_with_region(
            self.match_result.server_choice
        )
        ...
        if server_code:
            ...
            if region_info:
                ...
                if "americas" in region_name:
                    map_link = maps_service.get_map_battlenet_link(
                        map_short_name, "americas"
                    )
                elif "europe" in region_name:
                    map_link = maps_service.get_map_battlenet_link(
                        map_short_name, "europe"
                    )
        ...
        if self.match_result.match_result == "conflict":
            ...
            if self.match_result.match_result == "player1_win":
                ...
                print(
                    f"⚠️ EMBED: Unexpected match_result value: '{self.match_result.match_result}'"
                )
                result_display = "Draw"
            ...
            if p1_mmr_change is not None and p2_mmr_change is not None:
                ...
                mmr_display = (
                    f"- MMR Awarded: `{p1_display_name}: TBD`, `{p2_display_name}: TBD`"
                )
        else:
        ...
        embed.add_field(
            name="**Replay Status:**", value=replay_status_value, inline=False
        )
        ...
        p1_aborts = user_info_service.get_remaining_aborts(
            self.match_result.player_1_discord_id
        )
        p2_aborts = user_info_service.get_remaining_aborts(
            self.match_result.player_2_discord_id
        )
        ...
        embed.add_field(
            name="**Can't play? Need to leave?**",
            value=abort_validity_value,
            inline=False,
        )
    ...
    async def _send_final_notification_embed(self, final_results: dict):
        ...
        try:
            await self.last_interaction.followup.send(
                embed=notification_embed, ephemeral=False
            )
        except discord.HTTPException as e:
            print(
                f"Error sending final notification for match {self.match_result.match_id}: {e}"
            )
    ...
    async def _send_conflict_notification_embed(self):
        ...
        try:
            await self.last_interaction.followup.send(
                embed=conflict_embed, ephemeral=False
            )
        except discord.HTTPException as e:
            print(
                f"Error sending conflict notification for match {self.match_result.match_id}: {e}"
            )
    ...
    async def _send_abort_notification_embed(self):
        ...
        p1_info = db_reader.get_player_by_discord_uid(
            match_data["player_1_discord_uid"]
        )
        p2_info = db_reader.get_player_by_discord_uid(
            match_data["player_2_discord_uid"]
        )

        p1_name = (
            p1_info.get("player_name")
            if p1_info
            else str(match_data["player_1_discord_uid"])
        )
        p2_name = (
            p2_info.get("player_name")
            if p2_info
            else str(match_data["player_2_discord_uid"])
        )
        ...
        try:
            await self.last_interaction.followup.send(
                embed=abort_embed, ephemeral=False
            )
        except discord.HTTPException as e:
            print(
                f"Error sending abort notification for match {self.match_result.match_id}: {e}"
            )
...
class MatchAbortButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        success = matchmaker.abort_match(
            self.parent_view.match_result.match_id, player_discord_uid
        )
        ...
        if success:
            ...
            # Update button label
            remaining_aborts = user_info_service.get_remaining_aborts(
                player_discord_uid
            )
            ...
            if (
                self.parent_view.abort_disable_task
                and not self.parent_view.abort_disable_task.done()
            ):
            ...
            await interaction.response.send_message(
                "❌ Failed to abort match. It might have been already completed or aborted by the other player.",
                ephemeral=True,
            )
...
class MatchResultConfirmSelect(discord.ui.Select):
    ...
    def __init__(self, parent_view):
        ...
        options = [
            discord.SelectOption(label="Awaiting selection...", value="placeholder")
        ]
...
class MatchResultSelect(discord.ui.Select):
    ...
    def __init__(self, match_result: MatchResult, is_player1: bool, parent_view):
        ...
        self.p1_name = (
            p1_info.get("player_name")
            if p1_info
            else str(match_result.player_1_user_id)
        )
        ...
        self.p2_name = (
            p2_info.get("player_name")
            if p2_info
            else str(match_result.player_2_user_id)
        )

        # Create options for the dropdown
        options = [
            discord.SelectOption(label=f"{self.p1_name} victory", value="player1_win"),
            discord.SelectOption(label=f"{self.p2_name} victory", value="player2_win"),
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.parent_view.match_result.replay_uploaded != "Yes":
            await interaction.response.send_message(
                "❌ Please upload a replay file before reporting match results.",
                ephemeral=True,
            )
            return
        ...
        self.parent_view.confirm_select.options = [
            discord.SelectOption(label=f"Confirm: {result_label}", value="confirm")
        ]
    ...
    async def record_player_report(self, result: str):
        ...
        try:
            current_player_id = (
                ...
                if self.is_player1
                else self.parent_view.match_result.player_2_discord_id
            )
            success = matchmaker.record_match_result(
                self.match_result.match_id, current_player_id, report_value
            )

            if success:
                print(
                    f"📝 Player report recorded for match {self.match_result.match_id}. Waiting for backend notification."
                )
            else:
                print(
                    f"❌ Failed to record player report for match {self.match_result.match_id}"
                )
        except Exception as e:
    ...
    async def update_embed_with_mmr_changes(self):
        ...
        try:
            ...
            if (
                hasattr(self.parent_view, "last_interaction")
                and self.parent_view.last_interaction
            ):
    ...
    async def notify_other_player_result(self, original_embed, result_embed):
        ...
        for player_id in [
            ...
            if (
                other_view
                and hasattr(other_view, "last_interaction")
                and other_view.last_interaction
            ):
                try:
                    ...
                    await other_view.last_interaction.edit_original_response(
                        embeds=[original_embed, result_embed], view=other_view
                    )
                except:
                    pass  # Interaction might be expired

    async def notify_other_player_disagreement(
        self, original_embed, disagreement_embed
    ):
        ...
        for player_id in [
            ...
            if (
                ...
                try:
                    ...
                    # Update the other player's message
                    await other_view.last_interaction.edit_original_response(
                        embeds=[original_embed, disagreement_embed], view=other_view
                    )
                except:
                    pass  # Interaction might be expired
...
class BroodWarRaceSelect(discord.ui.Select):
    def __init__(self, default_value: Optional[str] = None):
        options = [
            ...
            for label, value, description in race_service.get_race_dropdown_groups()[
                "brood_war"
            ]
        ]
        super().__init__(
...
async def on_message(message: discord.Message, bot=None):
    ...
    try:
        ...
        print(
            f"[Main Process] Replay uploaded by {message.author.name} "
        ...
        try:
            if bot and hasattr(bot, "process_pool"):
                ...
                print(
                    "[WARN] Process pool not available, falling back to synchronous parsing"
                )
            ...
            print(
                f"[FATAL] Replay parsing in worker process failed: {type(e).__name__}: {e}"
            )
        ...
        if replay_info.get("error"):
            ...
            print(
                f"[Main Process] Worker process reported parsing error: {error_message}"
            )
            ...
            print(
                f"❌ Failed to parse replay for match {match_view.match_result.match_id}: {error_message}"
            )
            return
        ...
        if result.get("success"):
            ...
            print(
                f"✅ Replay file stored for match {match_view.match_result.match_id} (player: {message.author.id})"
            )
        else:
            ...
            print(
                f"❌ Failed to store replay for match {match_view.match_result.match_id}: {error_message}"
            )
