...
async def leaderboard_command(interaction: discord.Interaction):
    ...
    leaderboard_service = interaction.client.app_context.leaderboard_service
    try:
...
class CountryFilterPage1Select(discord.ui.Select):
    ...
    def __init__(self, countries, selected_countries=None):
        ...
        countries = self.view.leaderboard_service.country_service.get_common_countries()
        ...
        super().__init__(
            placeholder=placeholder, min_values=0, max_values=25, options=options, row=2
        )

    async def callback(self, interaction: discord.Interaction):
        self.view.leaderboard_service.update_country_filter(
            self.values, self.view.leaderboard_service.country_page2_selection
        )
...
class CountryFilterPage2Select(discord.ui.Select):
    ...
    def __init__(self, countries, selected_countries=None):
        ...
        super().__init__(
            placeholder=placeholder, min_values=0, max_values=25, options=options, row=3
        )

    async def callback(self, interaction: discord.Interaction):
        self.view.leaderboard_service.update_country_filter(
            self.view.leaderboard_service.country_page1_selection, self.values
        )
...
class RaceFilterSelect(discord.ui.Select):
    ...
    def __init__(self, leaderboard_service, selected_races=None):
        self.view = None  # Will be set when added to View
...
class LeaderboardView(discord.ui.View):
    ...
    def __init__(self, leaderboard_service: LeaderboardService):
        ...
        )  # Start disabled, will be enabled if not page 1
        self.add_item(
            NextPageButton(disabled=True)
        )  # Start disabled, will be enabled if not last page
        self.add_item(
            BestRaceOnlyButton(
                disabled=False, best_race_only=self.leaderboard_service.best_race_only
            )
        )  # Toggle for best race only mode
        self.add_item(ClearFiltersButton())

        # Add filter dropdowns (race first, then countries)
        self.add_item(
            RaceFilterSelect(
                self.leaderboard_service, self.leaderboard_service.race_filter
            )
        )
        self.add_item(
            CountryFilterPage1Select(
                self.countries, self.leaderboard_service.country_page1_selection
            )
        )
        self.add_item(
            CountryFilterPage2Select(
                self.countries, self.leaderboard_service.country_page2_selection
            )
        )
    ...
    async def update_view(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(
            embed=new_view.get_embed(data), view=new_view
        )
    ...
    def get_embed(self, data: dict = None) -> discord.Embed:
        ...
        embed = discord.Embed(
            title="🏆 Player Leaderboard", description="", color=discord.Color.gold()
        )
        ...
        pagination_info = self.leaderboard_service.get_pagination_info(
            total_pages, total_players
        )
...
class PreviousPageButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        if self.view.leaderboard_service.current_page > 1:
            self.view.leaderboard_service.set_page(
                self.view.leaderboard_service.current_page - 1
            )
            await self.view.update_view(interaction)
...
class BestRaceOnlyButton(discord.ui.Button):
    ...
    def __init__(self, disabled=False, best_race_only=False):
        ...
        super().__init__(
            label="Best Race Only", emoji=emoji, style=style, row=0, disabled=disabled
        )
