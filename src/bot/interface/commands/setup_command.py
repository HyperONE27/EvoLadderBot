...
from src.bot.interface.components.setup_modal import SetupModal


def register_setup_command(tree: app_commands.CommandTree):
    @tree.command(name="setup", description="Set up your player profile")
    async def setup(interaction: discord.Interaction):
        ...
        modal = SetupModal(
            validation_service=interaction.client.app_context.validation_service,
            user_info_service=interaction.client.app_context.user_info_service,
            countries_service=interaction.client.app_context.countries_service,
            regions_service=interaction.client.app_context.regions_service,
            player_data=player,
        )
...
class SetupModal(discord.ui.Modal, title="Player Setup"):
    def __init__(
        self, error_message: Optional[str] = None, existing_data: Optional[dict] = None
    ) -> None:
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        current_input = {
            ...
            "alt_player_name_1": (
                self.alt_id_1.value.strip() if self.alt_id_1.value else ""
            ),
            "alt_player_name_2": (
                self.alt_id_2.value.strip() if self.alt_id_2.value else ""
            ),
        ...
        if not is_valid:
            ...
            await send_ephemeral_response(
                interaction, embed=error_embed, view=ErrorView(error, current_input)
            )
            return
        ...
        if self.existing_data:
            ...
            if existing_country_code:
                try:
                    ...
                    existing_country = next(
                        (
                            c
                            for c in countries_service.get_common_countries()
                    ...
                    if existing_country:
                        ...
                        country_index = next(
                            (
                                i
                                for i, c in enumerate(common_countries)
                                if c["code"] == existing_country_code
                            ),
                            -1,
                        )
            ...
            if existing_region_code:
                existing_region = next(
                    (
                        r
                        for r in regions_service.get_all_regions()
        ...
        await send_ephemeral_response(interaction, embed=initial_embed, view=view)


class ErrorView(discord.ui.View):
    def __init__(
        self, error_message: str, existing_data: Optional[dict] = None
    ) -> None:
        super().__init__(timeout=GLOBAL_TIMEOUT)
        self.error_message = error_message
        self.existing_data = existing_data or {}
...
class CountryPage1Select(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        selected_country = next(
            c for c in self.view.countries if c["code"] == self.values[0]
        )
...
class RegionSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        selected_region = next(
            r for r in self.view.regions if r["code"] == self.values[0]
        )
...
class UnifiedSetupView(discord.ui.View):
    def __init__(
        ...
        self.country_page1_select = CountryPage1Select(
            self.countries, self.country_page1_selection
        )
        self.country_page2_select = CountryPage2Select(
            self.countries, self.country_page2_selection
        )
        self.region_select = RegionSelect(
            self.regions, self.selected_region["code"] if self.selected_region else None
        )
    ...
    async def update_view(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(
            embed=new_view.get_status_embed(), view=new_view
        )

    def get_status_embed(self) -> discord.Embed:
        """Get status embed based on current selections"""
        embed = discord.Embed(
            title="⚙️ Setup - Country & Region Selection", color=discord.Color.blue()
        )
    ...
    async def confirm_callback(self, interaction: discord.Interaction):
        ...
        if not self.selected_country or not self.selected_region:
            ...
            class SetupErrorView(discord.ui.View):
                ...
                @discord.ui.button(
                    emote="🔄", label="Try Again", style=discord.ButtonStyle.secondary
                )
                async def restart(
                    ...
                    await interaction.response.edit_message(
                        embed=embed, view=self.original_view
                    )

                @discord.ui.button(
                    emote="❌", label="Cancel", style=discord.ButtonStyle.danger
                )
                async def cancel(
                    self, interaction: discord.Interaction, button: discord.ui.Button
                ):
            ...
            await send_ephemeral_response(
                interaction, embed=error_embed, view=error_view
            )
            return
        ...
        await interaction.response.edit_message(content="", embed=view.embed, view=view)


def create_setup_confirmation_view(
    user_id: str, alt_ids: list, battle_tag: str, country: dict, region: dict
) -> ConfirmEmbedView:
    ...
    async def confirm_callback(interaction: discord.Interaction):
        ...
        post_confirm_view = ConfirmEmbedView(
            ...
            reset_target=SetupModal(
                existing_data=user_info_service.get_player(user_info["id"])
            ),
            restart_label="🔄 Setup Again",
        )

        await interaction.edit_original_response(
            content="", embed=post_confirm_view.embed, view=post_confirm_view
        )
