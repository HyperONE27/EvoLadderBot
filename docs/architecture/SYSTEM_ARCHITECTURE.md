...
2.  **Matchmaking**: The `MatchmakingService` periodically scans the queue. When a match is found, it calls `data_service.create_match`, which instantly adds a new row to the in-memory `_matches_1v1_df` DataFrame and enqueues the `INSERT` operation for the database.
